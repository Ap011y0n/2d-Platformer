...
bool j1App::Awake()
{
	...
	bool ret = true;
	if (result) { LOG("xml loaded %s", result.description()); }
	else {
	...
	while(item != NULL && ret == true)
	{
		...
		if (tool.select_single_node(item->data->name.GetString())) {
			node = &tool.select_single_node(item->data->name.GetString()).node();
			LOG("Node Pointer filled__ %s", node->name() );
		}
		else {
			node = nullptr;
			LOG("Node Pointer null");
		}
